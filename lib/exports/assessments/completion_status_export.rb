    ...
    class CompletionStatusExport
      ...
      def to_xlsx
        Axlsx::Package.new do |package|
          package.workbook.add_worksheet(name: 'CompletionStatus') do |sheet|
            ...
            header = ['Result ID',
                      'Name', 'Email',
                      'Assessment Type', 'Assessment Name', 'Started At', 'Completed At', 'Status']
      ...
      def subproject_level_assigns
        Queries::Assigns::SubProjectLevel::ByClient.call(@client_id).
          selecting do
          [id,
           original_assign.membership.user.last_name.
             op('||', quoted(', ')).op('||', original_assign.membership.user.first_name).as('user_name'),
