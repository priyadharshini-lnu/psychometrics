...
;(function ($) {
	var methods = {
			...
			var htmlContent = '<table class="'+options.tableClass+' kv-table">\
					...
					<tbody>\
						<tr class="'+options.templateClass+' kv-table-row">\
							<td><input type="text" class="'+options.keyInputClass.join(' ')+'" ></td>\
							<td><input type="text" class="'+options.valueInputClass.join(' ')+'" ></td>\
		...
		initData: function(options) {
			...
			for(var item in settings.initData) {
				$table.find('tbody>.'+settings.templateClass)
					...
					.find('.'+settings.keyInputClass.join('.'))
						.val(item)
						.attr('required',true)
						.end()
					.find('.'+settings.valueInputClass.join('.'))
		...
		updated: function(options){
			...
			$table.find('tbody tr').not('.'+settings.templateClass).each(function(){
				params[$(this).find('input.'+settings.keyInputClass.join('.')).val()] = $(this).find('input.'+settings.valueInputClass.join('.')).val();
			});
	...
	$.fn.kvJsonInput.defaults = {
		...
		keyInputClass: ['config-key', 'form-control'],
		valueInputClass: ['config-value', 'form-control'],
