...
module Administration
  class CampaignPolicy < Administration::BasePolicy
    ...
    def manage_project?
      @user.is?(:superadmin) || (@user.is?(:client_admin) && @user.has_client_grant?(:clients, :manage, @project_id))
    end

    def manage_threesixty?
      return true if @user.is?(:superadmin)
      return true if @user.is?(:client_admin, :project_admin) && @user.has_client_grant?(:clients, :manage, @project_id)
    ...
    def projects?
      @user.is?(:superadmin) || @user.has_client_grant?(:clients, :manage, @project_id)
    end
    ...
    def design?
      @user.is?(:superadmin) || @user.has_client_grant?(:clients, :design, @project_id)
    end
    ...
    def manage_messages?
      @user.is?(:superadmin) || @user.has_client_grant?(:campaigns, :manage_messages, @project_id)
    end
