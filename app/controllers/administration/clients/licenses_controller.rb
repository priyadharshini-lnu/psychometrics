  ...
  module Clients
    class LicensesController < Administration::BaseController
      ...
      def set_resource
        ...
        @_resource = policy_scope(resource_class).includes(licenses: %i[report_family license_usages]).find(resource_id)
      end

      def resource_params
        params.require(:resource).permit(:id, licenses_attributes: %i[id number overuse_number report_family_id
                                                                      start_date end_date _destroy disabled])
      end
