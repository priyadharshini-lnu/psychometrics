...
export const HIGH_PULSE_THRESHOLD = 0.8
...
export default function useAudioMetrics (
  recorder: React.MutableRefObject<RecorderCore | undefined>,
  ...
  const changeAudioLevel = () => {
    ...
    if (
      (level === AudioLevel.Low && batchPulsesRef.current.length)
      || performance.now() - lastAudiDetectorColorChangeRef.current
        > AUDIO_LEVEL_CHANGE_TO_LOW_THRESHOLD
    ) {
      const totalPulse = batchPulsesRef.current.length
      const pulseWithHighThreshold = filter(
        batchPulsesRef.current,
        (pulse: number) => pulse > HIGH_PULSE_THRESHOLD,
      ).length
      const percentOfHighPulse = (pulseWithHighThreshold / totalPulse) * 100
      setLevel(
        percentOfHighPulse > PERCENT_OF_HIGH_PULSE_THRESHOLD
          ? AudioLevel.High
          : AudioLevel.Low,
      )
      lastAudiDetectorColorChangeRef.current = performance.now()
