...
class CampaignFactors extends BaseType {
  isFilled () {
    const {
  ...
  compareAggregatedData () {
    ...
    const factorResults = this.getResults().campaignFactorResults
    const value = factorResults && _.find(factorResults, { code: subject })?.value
