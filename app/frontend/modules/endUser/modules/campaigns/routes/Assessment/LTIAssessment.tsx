...
import useLtiLaunchRequest from '~/hooks/useLtiLaunchRequest'
...
export const LTIAssessment: FC<Props> = ({
  userAssessmentId,
  userAssessmentUrl,
  onCancel,
}) => {
  const { loading, makeLtiLaunchRequest } = useLtiLaunchRequest({
    url: userAssessmentUrl,
  })

  const process = async () => {
    await makeLtiLaunchRequest({ id: userAssessmentId })
  }
