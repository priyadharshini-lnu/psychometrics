...
interface Props {
  ...
  uploadFiles: (id: string, data: FormData) => Promise<unknown>
}

export const DesignSettingsForm: React.FC<Props> = ({
  entityId, entityName, entityType, uploadFiles,
}) => {
  ...
  useEffect(() => {
    if (designSettings) {
      form.setFieldsValue({
        ...designSettings,
        logoAltText: designSettings.logoAltText || entityName,
        secondaryLogoAltText: designSettings.secondaryLogoAltText || entityName,
      })
      setIsLoading(false)
    }
  }, [designSettings, entityName])

  useEffect(() => {
    fetch({
      apiConfig: {
        filter: entityType === 'client'
          ? { client_id_eq: entityId }
