...
export const ReflectionQuestionsFilter: React.FC = () => {
  ...
  const handleImport = (data:FormData, projectId: number|null, successCallback:()=>void, failureCallback:(error)=>void) => {
    ...
    resource.uploadFileAction(action, data).then(() => {
      successCallback()
      message.info(I18n.t('admin.reflection_questions_import_success_msg'))
    })
      .catch((error) => {
        failureCallback(error)
        message.info(I18n.t('admin.reflection_questions_import_failure_msg'))
      })
  }
  ...
  const handleActionExport = (projectId: number) => {
    resource.collectionAction({
      ...
      message.info(I18n.t('admin.reflection_questions_export_success_msg'))
    })
      .catch(() => {
        message.error(I18n.t('admin.reflection_questions_export_failure_msg'))
      })
  }
  ...
  const handleToolAction = (action: string) => {
    if (action === 'import') {
      dispatch(openModal('ImportModal', {
        handleImport,
        title: I18n.t('admin.reflection_questions_import_action'),
      }))
    }
