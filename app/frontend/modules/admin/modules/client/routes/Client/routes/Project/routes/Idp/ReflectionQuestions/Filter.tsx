...
export const ReflectionQuestionsFilter: React.FC = () => {
  ...
  const handleImport = (data:FormData, projectId: number|null, successCallback:()=>void, failureCallback:(error)=>void) => {
    const action = `reflection_questions/import?project_id=${projectId}`

    resource.uploadFileAction(action, data).then(() => {
      successCallback()
      message.info(I18n.t('administration.reflection_questions.import.success_msg'))
    })
      .catch((error) => {
        failureCallback(error)
        message.info(I18n.t('administration.reflection_questions.import.failure_msg'))
      })
  }
  ...
  const handleActionExport = (projectId: number) => {
    resource.collectionAction({
      ...
      message.info(I18n.t('administration.reflection_questions.export.success_msg'))
    })
      .catch(() => {
        message.error(I18n.t('administration.reflection_questions.export.failure_msg'))
      })
  }
  ...
  const handleToolAction = (action: string) => {
    if (action === 'import') {
      dispatch(openModal('ImportModal', {
        handleImport,
        title: I18n.t('administration.reflection_questions.import.action'),
      }))
    }
