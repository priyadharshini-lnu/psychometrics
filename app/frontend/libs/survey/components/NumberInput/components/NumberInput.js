...
class NumberInput extends Component {
  ...
  render () {
    ...
    const val = this.isNumber(value) ? parseFloat(value).toPrecision(1) : this.getMinValue()

    return (
      <div className={styles.inputContainer}>
        <div className={styles.label}>{label}</div>
        <div className={styles.numberInput}>
          <a className={styles.control} onClick={this.decrement}>
            <span className={`fa fa-minus ${styles.icon}`} />
          </a>
          <input onChange={this.change} value={val} className={`from-control ${styles.input}`} />
          <a className={styles.control} onClick={this.increment}>
