...
import { getValidationKey, isEmailTextEntryQuestion } from 'libs/survey/utils/question'
...
const TextEntryValidation = {
  run (moduleResult: ModuleResult) {
    ...
    if (_.isEmpty(to)) {
      errors.push({
        type: 'forceRequired',
        message: Watchman.I18n().t(`${getValidationKey(question)}.to.required`),
        field: 'to',
      })
    }
    ...
    if (!subject || subject.length < 10) {
      errors.push({
        type: 'forceRequired',
        message: Watchman.I18n().t(`${getValidationKey(question)}.subject.min_length`),
        field: 'subject',
      })
    }
