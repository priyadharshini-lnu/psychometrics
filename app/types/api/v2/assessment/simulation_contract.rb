...
module Api
  module V2
    module Assessment
      class SimulationContract < Api::Base::Contract
        rule(data: { attributes: :category }) do
          next unless value

          list = [::Assessment::SIMULATION]
        ...
        rule(data: { attributes: { external_settings: :assessment_id } }) do
          ...
          key.failure(:not_in_the_list?) unless Settings.providers.simulation.assessments.map(&:id).include?(value)
        end
      end
    end
  end
