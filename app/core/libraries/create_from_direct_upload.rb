...
module Libraries
  class CreateFromDirectUpload < BaseCommand
    ...
    def call
      ...
      checksum = resolve_checksum(client, temporary_upload)
    ...
    def build_library_resource(temporary_upload)
      ...
      resource.owner_id = if current_user.is?(:client_admin) && params[:owner_id].blank?
                            current_user.client_admin_client_ids.first
                          else
    ...
    def attach_file_to_library!(library, temporary_upload, checksum)
