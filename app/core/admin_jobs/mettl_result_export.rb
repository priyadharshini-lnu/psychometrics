  ...
  class MettlResultExport < BaseExportAssessment
    ...
    def headers
      [
        ...
        'Relationship', 'Started At', 'Completed At', 'Status', 'Max Marks', 'Percentile',
        'Total Marks', 'Attempt Time'
      ]
    end

    def data_row(user_result)
      external_results = RESULT_HEADERS_DATA.map { |header| user_result.external_results&.dig('scores', header) }
