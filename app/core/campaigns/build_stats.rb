  ...
  class BuildStats < BaseCommand
    ...
    def build_users
      query = CampaignUser.select(:status, 'count(id) as status_count').
              group(:status).where(campaign_id: campaign.id).to_sql
      ActiveRecord::Base.connection.execute(query).to_a.
        each_with_object({ 'total' => campaign.campaign_users.count }) do |row, res|
        res[UserAssessment.statuses.key(row['status'])] = row['status_count']
      end
    end

    def build_assessments
      query = UserAssessment.select(:assessment_id, :status, 'count(id) as status_count').
              group(:assessment_id, :status).
              where(campaign_id: campaign.id).to_sql
