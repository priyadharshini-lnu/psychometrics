    ...
    module ProjectLevel
      class BulkReportWithOptions < ::Queries::Base
        ...
        def without_norm_data
          @relation.where('assigns.norm_data is NULL').or(@relation.where("assigns.norm_data->>'type' is NULL"))
        end

        def with_norm_data
          @relation.where('assigns.norm_data is NOT NULL').where("assigns.norm_data->>'type' is NOT NULL")
        end
