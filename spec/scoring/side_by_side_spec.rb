...
RSpec.describe Scoring::SideBySide do
  ...
  let!(:template_data) do
    [
      { 'scale' => 0, 'choice' => 0, 'values' => [
        { 'index' => 0, 'value' => 1 },
        { 'index' => 1, 'value' => 6 }
      ] },
      { 'scale' => 1, 'choice' => 0, 'values' => [{ 'index' => 0, 'value' => 1 }, { 'index' => 1, 'value' => 2 }] },
      { 'scale' => 0, 'choice' => 1, 'values' => [{ 'index' => 0, 'value' => 1 }, { 'index' => 1, 'value' => 6 }] },
      { 'scale' => 1, 'choice' => 1, 'values' => [{ 'index' => 0, 'value' => 1 }, { 'index' => 1, 'value' => 2 }] },
      { 'scale' => 0, 'choice' => 2, 'values' => [{ 'index' => 0, 'value' => 1 }, { 'index' => 1, 'value' => 6 }] },
    ...
    context 'when scoring is ... (show template data)' do
      context 'when answer: s0c0i1, s1c2i0, s1c2i1' do
        it 'returns 1.3333' do
          result = side_by_side.calculate(question, { 'answers' => [
            { 'scale' => 0, 'choice' => 0, 'values' => [
              { 'index' => 1, 'value' => true }
            ] },
          ...
          expect(result[:max_value]).to eq(18)
        end
      end

      context 'when no answer' do
        it 'returns nil' do
          result = side_by_side.calculate(question, { 'answers' => [] }, factors_scoring(template_data))[:value]
      ...
      context 'when answer: {\'scale\' => 0, \'choice\' => 0, \'values\' => []}' do
        it 'returns nil' do
          result = side_by_side.calculate(question,
