  ...
  describe '#calculate' do
    context 'when scoring set from template_data (see template_data)' do
      context 'when answer: ruby' do
        it 'returns 5' do
          result = text_entry.calculate(Question.new, { 'answers' => [{ 'value' => 'ruby' }] },
      ...
      context 'when answer: java' do
        it 'returns nil' do
          result = text_entry.calculate(Question.new, { 'answers' => [{ 'value' => 'java' }] },
                                        factors_scoring(template_data))[:value]
      ...
      context 'when no answer' do
        it 'returns nil' do
          result = text_entry.calculate(Question.new, { 'answers' => [{ 'value' => '' }] },
                                        factors_scoring(template_data))[:value]
          expect(result).to be_nil
        end
      end
