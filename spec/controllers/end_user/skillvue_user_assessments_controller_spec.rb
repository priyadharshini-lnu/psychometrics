...
RSpec.describe EndUser::SkillvueUserAssessmentsController, type: :controller do
  ...
  let(:user_assessment) do
    create(:user_assessment,
           ...
           skillvue_user_assessment: build(:skillvue_user_assessment))
  end
  let(:campaign) { user_assessment.campaign }
  let(:async_request_uuid) { "#{uuid}|#{user.id}" }
  let(:request_params) { { id: user_assessment.id, skillvue_user_assessment: { id: user_assessment.id } } }
