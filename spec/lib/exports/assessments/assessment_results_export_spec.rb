...
describe 'Exports::Assessments::AssessmentResultsExport' do
  let(:assessment_id) { 0 }
  let(:client_id) { 0 }
  let(:exporter) { Exports::Assessments::AssessmentResultsExport.new(client_id, assessment_id) }

  describe '#current_level_assigns' do
    before do
      expect(Client).to receive(:find).with(client_id).and_return(project)
      expect(query_object).to receive(:call).with(client_id, assessment_id).and_return(Assign.none)
    end
