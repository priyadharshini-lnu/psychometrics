...
describe Communications::ReminderType::NotStartedJob, type: :job do
  let(:campaign) { create(:campaign) }

  let(:project) { campaign.project }
  let!(:campaign_user1) { create(:campaign_user, campaign: campaign, completion_status: :not_started) }
  let!(:campaign_user2) { create(:campaign_user, campaign: campaign, completion_status: :not_started) }
  ...
  it 'creates reminder communication email for users who completion_status in not_started' do
    expect do
