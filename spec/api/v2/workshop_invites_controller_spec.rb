...
RSpec.describe Api::V2::Administration::WorkshopInvitesController, type: :request do
  ...
  describe 'POST /campaigns/:campaign_id/workshop_invites/validate_subjects' do
    ...
    it 'validates subjects with no conflicts' do
      body = {
        data: {
          type: 'workshop_invites',
          attributes: {
      ...
      post "/api/v2/administration/campaigns/#{campaign_id}/workshop_invites/validate_subjects",
           params: body.to_json,
           headers: { 'Authorization' => authorization, 'Content-Type' => 'application/vnd.api+json' }
    ...
    it 'returns conflicts when users are already invited to assessment group' do
    ...
    it 'ignores conflicts from different assessment groups' do
      other_assessment_group = create(:campaign_assessment_group, campaign: campaign)
      other_workshop_invite = create(:workshop_invite,
                                     campaign: campaign,
