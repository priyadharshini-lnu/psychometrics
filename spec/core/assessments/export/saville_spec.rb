...
describe Assessments::Export::Saville do
  ...
  def verify_nipsitive_sheet(sheet)
    expected_headers = ['Result ID', 'Full Name', 'Email', 'Started at', 'Completed at',
    ...
    expected_sub_headers = [nil, nil, nil, nil, nil, @nipsitive_factor1.name, @nipsitive_factor2.name]
    result_sub_headers = sheet.row(2)
    expect(result_sub_headers).to eq(expected_sub_headers)

    expected_value_type_headers = [nil, nil, nil, nil, nil, @nipsitive_factor1.value_type,
  ...
  def verify_ipsitive_sheet(sheet)
    ...
    expected_sub_headers = [nil, nil, nil, nil, nil, @ipsitive_factor1.name, @ipsitive_factor2.name]
    result_sub_headers = sheet.row(2)
    expect(result_sub_headers).to eq(expected_sub_headers)

    expected_value_type_headers = [nil, nil, nil, nil, nil, @ipsitive_factor1.value_type, @ipsitive_factor2.value_type]
    result_value_type_header = sheet.row(3)
    expect(result_value_type_header).to eq(expected_value_type_headers)
