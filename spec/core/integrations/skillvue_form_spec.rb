...
describe Integrations::SkillvueForm do
  let(:attributes) do
    {
      name: 'skillvue',
  ...
  describe '#api_key' do
    ...
    it 'encrypts and Base64 encodes the API key' do
      ...
      form = described_class.new(api_key: api_key, name: 'skillvue')
      expect(form.api_key).to eq('base64-encrypted-api-key')
    end

    it 'does not encrypt if api_key is blank' do
      form = described_class.new(api_key: '', name: 'skillvue')
  ...
  describe 'integration uniqueness' do
    it 'is invalid if skillvue integration is already present for same project' do
      create(:integration, name: :skillvue, project: project)
      form = described_class.new(attributes).with_context(project: project)
    ...
    it 'allows updating the same integration' do
      existing_integration = create(:integration, name: :skillvue, project: project)
  ...
  describe 'unique_api_key validation' do
    let(:existing_integration) do
      create(:integration, name: :skillvue, config: { api_key: Base64.encode64(Encryptor.encrypt('test-api-key')) })
    end

    it 'is invalid if api_key is already used by another skillvue integration' do
      existing_integration
      form = described_class.new(attributes).with_context(project: project)

      expect(form).not_to be_valid
      expect(form.errors[:base]).to include(I18n.t('administration.integrations.validations.skillvue.api_key_present'))
    end

    it 'allows using a different api_key even when other skillvue integrations exist' do
      existing_integration
