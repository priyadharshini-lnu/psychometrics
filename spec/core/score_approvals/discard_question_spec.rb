# frozen_string_literal: true

require 'rails_helper'

RSpec.describe ScoreApprovals::DiscardQuestion do
  subject(:service) { described_class.call(score_approval, question_id, current_user) }

  let(:campaign) { create(:campaign) }
  let(:assessment) { create(:assessment) }
  let(:users_result) { create(:users_result) }
  let!(:user_assessment) do
    create(:user_assessment, campaign: campaign, assessment: assessment, users_result: users_result)
  end
  let!(:question) { create(:question, assessment: assessment) }
  let!(:question2) { create(:question, assessment: assessment) }
  let(:question_id) { question.id }
  let(:question2_id) { question2.id }
  let(:current_user) { create(:user) }
  let(:competency_factor) { create(:factor) }
  let(:assessor) { create(:user) }
  let(:approver) { create(:user) }
  let!(:score_approval) { AI::ScoreApproval.find(user_assessment.id) }

  let!(:approval_settings) do
    create(:ai_scoring_approval_setting,
           campaign_id: campaign.id,
           assessment_id: assessment.id,
           assessor_ids: [assessor.id],
           approver_ids: [approver.id])
  end

  let!(:competency) do
    create(:ai_factor_score,
           users_result: users_result,
           factor: competency_factor,
           question_id: nil,
           status: :pending,
           scoring_type: :aggregated,
           parent_factor_id: nil)
  end

  let!(:indicators) do
    create_list(:ai_factor_score, 3,
                users_result: users_result,
                question_id: question_id,
                status: :assessor_approved,
                parent_factor_id: competency_factor.id,
                scoring_type: :ai,
                override_score: 3)
  end

  let!(:other_question_scores) do
    create_list(:ai_factor_score, 2,
                users_result: users_result,
                question_id: question2_id,
                status: :pending,
                parent_factor_id: competency_factor.id,
                scoring_type: :ai)
  end

  describe '#call' do
    context 'when user is not allowed to discard' do
      let(:unauthorized_user) { create(:user) }
      let(:current_user) { unauthorized_user }

      it 'returns error when user is not in assessor or approver list' do
        result = service

        expect(result[:error]).to eq(I18n.t('admin.score_approval_not_allowed_to_discard'))
        expect(result[:ok]).to be_nil
      end

      it 'does not update any question scores' do
        service

        indicators.each do |score|
          expect(score.reload.status).to eq('assessor_approved')
          expect(score.reload.override_score).to eq(3)
        end
      end
    end

    context 'when current user is an assessor' do
      let(:current_user) { assessor }

      context 'with two-level approval settings' do
        it 'discards question scores and resets override values' do
          result = service

          expect(result[:ok]).to include(*indicators)
          indicators.each do |score|
            score.reload
            expect(score.override_score).to be_nil
            expect(score.not_applicable).to be false
          end
        end

        it 'does not change other question scores' do
          service

          other_question_scores.each do |score|
            score.reload
            expect(score.status).to eq('pending')
          end
        end
      end
    end

    context 'when current user is an approver' do
      let(:current_user) { approver }

      before do
        score_approval.update!(approval_status: :assessor_approved)
      end

      context 'with two-level approval settings' do
        it 'discards question scores and resets override values' do
          result = service

          expect(result[:ok]).to include(*indicators)
          indicators.each do |score|
            score.reload
            expect(score.override_score).to be_nil
            expect(score.not_applicable).to be false
          end
        end
      end

      context 'with one-level approval settings' do
        before do
          approval_settings.update!(allow_one_level_approve: true)
          score_approval.update!(approval_status: :pending)
        end

        it 'allows approver to discard question scores' do
          result = service

          expect(result[:ok]).to include(*indicators)
          indicators.each do |score|
            score.reload
            expect(score.override_score).to be_nil
          end
        end
      end
    end

    context 'when question has scores with not_applicable flag' do
      let(:current_user) { assessor }

      before do
        indicators.first.update!(not_applicable: true)
      end

      it 'resets not_applicable flag to false' do
        service

        expect(indicators.first.reload.not_applicable).to be false
      end
    end

    context 'when question has overridden scores' do
      let(:current_user) { assessor }

      before do
        indicators.each { |score| score.update!(override_score: 5) }
      end

      it 'clears the override_score' do
        service

        indicators.each do |score|
          expect(score.reload.override_score).to be_nil
        end
      end
    end

    context 'with aggregated scores' do
      let!(:aggregated_scores) do
        create_list(:ai_factor_score, 2,
                    users_result: users_result,
                    question_id: question_id,
                    status: :assessor_approved,
                    scoring_type: :aggregated)
      end

      let(:current_user) { assessor }

      it 'includes aggregated scores in the discard operation' do
        result = service

        expect(result[:ok]).to include(*indicators, *aggregated_scores)
      end
    end

    context 'when user is superadmin' do
      let(:current_user) { create(:superadmin) }

      before do
        indicators.each { |score| score.update!(status: :approver_approved, override_score: 5) }
      end

      context 'with pending status' do
        before do
          score_approval.update!(approval_status: :pending)
        end

        it 'allows discard operation' do
          result = service

          expect(result[:ok]).to include(*indicators)
          indicators.each do |score|
            score.reload
            expect(score.override_score).to be_nil
          end
        end
      end

      context 'with assessor_approved status' do
        before do
          score_approval.update!(approval_status: :assessor_approved)
        end

        it 'allows discard operation' do
          result = service

          expect(result[:ok]).to include(*indicators)
          indicators.each do |score|
            score.reload
            expect(score.override_score).to be_nil
          end
        end
      end

      context 'with approver_approved status' do
        before do
          score_approval.update!(approval_status: :approver_approved)
        end

        it 'allows discard operation (superadmin override)' do
          result = service

          expect(result[:ok]).to include(*indicators)
          indicators.each do |score|
            score.reload
            expect(score.override_score).to be_nil
          end
        end
      end
    end

    context 'when policy denies discard' do
      let(:current_user) { approver }

      before do
        score_approval.update!(approval_status: :pending)
        approval_settings.update!(allow_one_level_approve: false)
      end

      it 'returns error message' do
        result = service

        expect(result[:error]).to eq(I18n.t('admin.score_approval_not_allowed_to_discard'))
        expect(result[:ok]).to be_nil
      end

      it 'does not update question scores' do
        service

        indicators.each do |score|
          expect(score.reload.status).to eq('assessor_approved')
          expect(score.reload.override_score).to eq(3)
        end
      end
    end

    context 'when trying to discard with wrong role for current status' do
      context 'approver trying to discard pending status in two-level approval' do
        let(:current_user) { approver }

        before do
          approval_settings.update!(allow_one_level_approve: false)
          score_approval.update!(approval_status: :pending)
        end

        it 'returns error' do
          result = service

          expect(result[:error]).to eq(I18n.t('admin.score_approval_not_allowed_to_discard'))
        end
      end

      context 'assessor trying to discard assessor_approved status' do
        let(:current_user) { assessor }

        before do
          score_approval.update!(approval_status: :assessor_approved)
        end

        it 'returns error' do
          result = service

          expect(result[:error]).to eq(I18n.t('admin.score_approval_not_allowed_to_discard'))
        end
      end
    end
  end
end
