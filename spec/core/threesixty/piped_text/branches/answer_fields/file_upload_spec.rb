...
describe Threesixty::PipedText::Branches::AnswerFields::FileUpload do
  describe '.call' do
    ...
    it 'doc file' do
      allow(result).to receive_message_chain(:media_responses, :find_by) { media_response }
      allow(media_response).to receive_message_chain(:asset, :url) {
                                 'https://lvh.me:3030/uploads/media_response/asset/prometeus.doc'
                               }
      ...
      expect(response).to eq('<iframe style="width: 100%; height: 400px; background: black; border: none;"'\
        ' src="https://view.officeapps.live.com/op/embed.aspx?src=https://lvh.me:3030/uploads/media_response/asset/'\
