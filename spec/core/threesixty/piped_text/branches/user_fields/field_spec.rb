...
describe Threesixty::PipedText::Branches::UserFields::Field do
  describe '.call' do
    ...
    it do
      response = described_class.call!(%w[Field Name], {}, threesixty_campaign: 'ddd', user: user)
      expect(response).to eq('Vasiliy Pupkin')
    end

    it do
      response = described_class.call!(%w[Field Email], {}, threesixty_campaign: 'ddd', user: user)
      expect(response).to eq('my@email.com')
    end

    it do
      response = described_class.call!(%w[Field FirstName], {}, threesixty_campaign: 'ddd', user: user)
      expect(response).to eq('Vasiliy')
    end

    it do
      response = described_class.call!(%w[Field Oops], {}, threesixty_campaign: 'ddd', user: user)
