...
describe Examus::FindOrCreateSession do
  ...
  let(:proctoring_license) { create(:proctoring_license, is_project_specific: false) }
  ...
  context 'with project-specific license' do
    let(:project) { campaign_user.campaign.project }
    ...
    it 'creates proctoring session and deducts both license and project_license' do
      ...
      expect(campaign_user.campaign).to receive(:proctoring_license_with_enough_credits).
        and_return(project_specific_license)
    ...
    it 'returns error if project_license is disabled' do
      ...
      expect(result[:error]).to include('Proctoring License')
    ...
    it 'returns error if project_license does not have enough credits' do
      ...
      expect(campaign_user.campaign).to receive(:proctoring_license_with_enough_credits).
        and_return(project_specific_license)
      expect(Campaigns::Proctoring::GetProctoringCredits).to receive(:call!).and_return(10).once

      result = described_class.call(campaign_user)

      expect(result[:error]).to include("Usage limit reached for 'Proctoring License', please contact administrator")
      expect(ProctoringSession.count).to eq(0)
    end
  ...
  context 'without enough license credits' do
    ...
    it 'returns error if project_license does not exist for project-specific license' do
      project_specific_license = create(:proctoring_license, is_project_specific: true,
client: campaign_user.campaign.client)
      expect(campaign_user.campaign).to receive(:proctoring_license_with_enough_credits).
        and_return(project_specific_license)
