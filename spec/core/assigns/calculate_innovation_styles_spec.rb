...
describe Assigns::CalculateInnovationStyles do
  ...
  context '#condition_valid?' do
    context 'equal_to' do
      ...
      it 'pass' do
        result = described_class.new(assign).send(:condition_valid?, innovation_styles_first_factor_one, 3.0)
        expect(result).to be_truthy
      end

      it 'not pass' do
        result = described_class.new(assign).send(:condition_valid?, innovation_styles_first_factor_one, 6.0)
