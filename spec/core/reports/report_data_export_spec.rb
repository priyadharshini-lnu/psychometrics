...
describe Reports::ExportData do
  describe '.export' do
    ...
    let(:user)             { double('user', first_name: 'Jon', last_name: 'Snow', id: 1) }
    let(:aliases)          { double('aliases') }
    let(:user_result) do
      double('user_result', subject: user,
                       evaluator: user,
                       subject_id: user.id,
    ...
    context '#to_xlsx' do
      ...
      it do
        allow_any_instance_of(described_class).to receive(:fetch_factor_label).with(factor.id).and_return(factor.name)
        allow_any_instance_of(described_class).to receive(:users_results).and_return([user_result])
