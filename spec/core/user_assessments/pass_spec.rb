...
describe UserAssessments::Pass do
  ...
  it 'status = not_started' do
    user_assessment.users_result.status = 'not_started'
    described_class.call!(user_assessment, 'ru')

    expect(user_assessment.users_result.status).to eq('in_progress')
  ...
  it 'status = interrupted' do
    user_assessment.users_result.status = 'interrupted'
  ...
  it 'status = completed' do
    user_assessment.users_result.status = 'completed'
    described_class.call!(user_assessment, nil)
