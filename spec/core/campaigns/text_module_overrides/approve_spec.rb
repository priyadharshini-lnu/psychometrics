...
describe Campaigns::TextModuleOverrides::Approve do
  ...
  let(:module_override) { create(:text_module_override, module_id: report_module.id) }

  describe 'broadcast ok' do
    it 'should approve with new record' do
      text_override = described_class.call!({ module_id: report_module.id }, current_user)
      expect(text_override.content).to eq nil
      expect(text_override.approved).to eq true
    end

    it 'should approve with existing record' do
      described_class.call!({ id: module_override.id, module_id: report_module.id }, current_user)
