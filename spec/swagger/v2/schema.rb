...
module Swagger
  module V2
    DYNAMIC_SCHEMA_FILE = "#{__dir__}/dynamic_schema.json"
    DYNAMIC_SCHEMA = JSON.parse(File.read(DYNAMIC_SCHEMA_FILE))
    DEFINITION = {
      openapi: '3.0.1',
      info: {
      ...
      components: {
        ...
        }.merge(DYNAMIC_SCHEMA)
      }
    }

    def self.dynamic_schema # rubocop:disable Metrics/AbcSize
      # Once you add any schema here please run rake generate_dynamic_swagger_schema and commit dynamic_schema.json
      {
    ...
    def self.generated_dynamic_schema
      File.read('./dynamic_schema.json')
    end

    def self.generate_dynamic_schema
      File.write(DYNAMIC_SCHEMA_FILE, JSON.pretty_generate(dynamic_schema))
    end
  end
end

Swagger::V2.generate_dynamic_schema if Rails.env.test? && ENV.fetch('CI_RUN', 'false') == 'false'
