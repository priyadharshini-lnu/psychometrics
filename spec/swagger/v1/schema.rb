    ...
    DEFINITION = {
      ...
      info: {
        ...
        <<~DESCRIPTION
          ...
          ### Bearer JWT (RS256)
          REST API requests can be authenticated using a short-lived JWT passed as a Bearer token in the `Authorization` header.
          ...
          |-------|-------------|-------------------- |
          | `iss` | Your Lighthouse Application ID | Required |
          | `jti` | Unique token identifier (UUID recommended) | Optional |
          | `aud` | Client project URL in subdomain + domain format, e.g. `https://client.tte-lighthouse.com` | Required |
          | `exp` | Token expiry (Unix timestamp). Recommended: less than 5 minutes. Maximum: 1 hour | Required |

          #### Following are the claims that should be present in the header:
          - `alg`: Use RS256
          - `kid`: RS256 key ID identifying the signing key
          - `typ`: `JWT`
          ...
          Lighthouse supports participant SSO via a signed RS256 JWT submitted as a form POST.
          This method keeps the token out of browser history, URL query strings, referrer headers, and access logs.
          ...
          **Request:** Submit the token as a form field named `token`.
          ...
          token=<jwt_token>
          ```
          The JWT token should be signed with the Private Key.

          #### Following are the claims that should be present in the payload:

          | Field | Description | Required / Optional |
          |-------|-------------|----------|
          | `iss` | Lighthouse Application ID | Required |
          | `jti` | Unique token identifier (UUID recommended) | Optional. Required if single_use is true  |
          | `aud` | Client project URL in subdomain + domain format, e.g. `https://client.tte-lighthouse.com` | Required |
          | `sub` | Lighthouse User's email address or User ID  | Required |
          | `exp` | Token expiry (Unix timestamp). Recommended: less than 5 minutes. Maximum: 1 hour | Required |
          | `tg` | Target type: `cmp` (campaign) or `asmt` (assessment) | Optional. Required when ret_url is specified  |
          | `tg_cmp_id` | Target Campaign ID. The campaign must belong to the signing Application's tenant | Required if tg is cmp/asmt  |
          | `tg_asmt_id` | Target Assessment ID | Required if tg is asmt |
          | `ret_url` | Return URL to redirect the user after completion of the activities | Optional |
          | `single_use` | Invalidate token after single use | Optional. Default false |
          ...
          #### Single-Use Tokens
          When single_use is set to true, the iss + jti combination is recorded on first use and subsequent requests with the same
          token are rejected until the token expires. When single_use is false or omitted, the token may be reused until it expires.

          #### Validation Rules
          - The `aud` claim must match the client project URL using subdomain + domain format only. Path-based audience values are rejected.
          - Tokens signed with algorithms other than `RS256` are rejected.
          - Tokens with an expiry greater than 1 hour are rejected.
          - The `kid` must identify an active public key configured for the Application identified by `iss`.

          #### Return URL Placeholder Substitution
          The `ret_url` may contain the placeholder `ASSESSMENT_STATUS`, e.g. `https://webhook.cool/?status=ASSESSMENT_STATUS`.
          Lighthouse substitutes this placeholder with one of the following values upon redirect:
          ...
          generic authentication failure response is returned. Invalid or unverified tokens never result in a redirect to `ret_url`.
