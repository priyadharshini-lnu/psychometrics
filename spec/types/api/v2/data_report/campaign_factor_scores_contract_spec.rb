...
RSpec.describe Api::V2::DataReport::CampaignFactorScoresContract do
  ...
  describe 'report_type validation' do
    ...
    it 'fails when report_type is not campaign_factor_scores' do
      params = valid_params
  ...
  describe 'configuration validation' do
    it 'fails when configuration is invalid JSON' do
      ...
      expect(error_message).to include(I18n.t('administration.data_reports.validations.invalid_json'))
    end

    it 'fails when project_ids is empty for client scope' do
    ...
    it 'succeeds when project_ids is empty for global scope' do
    ...
    it 'fails when project_ids contains non-existent projects' do
      ...
      expect(error_message).to include(
        I18n.t('admin.invalid_project_ids', ids: '999999')
      )
    end
  end
  ...
  describe 'client scope validation' do
    ...
    it 'fails when project does not belong to the client' do
      ...
      error_message = result.errors.to_hash.values.flatten.join(' ')
      expect(error_message).to include(
        I18n.t('admin.project_not_related_to_client',
