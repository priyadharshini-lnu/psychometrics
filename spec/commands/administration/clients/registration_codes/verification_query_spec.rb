...
describe Administration::Clients::RegistrationCodes::VerificationQuery do
  ...
  let(:upcoming_registration_code) { create(:registration_code, start_date: 15.days.from_now) }
  let(:expired_registration_code) do
    create(:registration_code, start_date: 2.days.ago, end_date: 1.day.ago)
  end
  ...
  context 'Execution' do
    ...
    it 'does not return an expired registration_code' do
      allow(Time).to receive(:now).and_return(Time.zone.local(2022, 1, 8, 0, 0, 0))
