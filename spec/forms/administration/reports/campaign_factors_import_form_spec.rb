...
RSpec.describe Administration::Reports::CampaignFactorsImportForm do
  let(:report) { create(:report) }
  let(:report_id) { report.id }

  describe 'validations' do
    it 'validates presence of report_id and file' do
      form = described_class.new
      expect(form.valid?).to be false
      expect(form.errors[:report_id]).to include("can't be blank")
    ...
    it 'validates file content type' do
      ...
      form = described_class.new(file: file, report_id: report_id)
      expect(form.valid?).to be false
    ...
    it 'validates that report exists' do
      ...
      form = described_class.new(file: file, report_id: -1)
      expect(form.valid?).to be false
      expect(form.errors[:report]).to include('Report must exist')
    end

    it 'validates required headers' do
      file = Rack::Test::UploadedFile.new(
        Rails.root.join('spec/fixtures/files/reports/import_campaign_factors/missing_header.xlsx'),
        'application/xlsx'
      )
      form = described_class.new(file: file, report_id: report_id)
      expect(form.valid?).to be false
    ...
    it 'validates output type values' do
      file = Rack::Test::UploadedFile.new(
        Rails.root.join('spec/fixtures/files/reports/import_campaign_factors/invalid_output_type.xlsx'),
        'application/xlsx'
      )
      form = described_class.new(file: file, report_id: report_id)
      expect(form.valid?).to be false
  ...
  def valid_excel_file
    Rack::Test::UploadedFile.new(
      Rails.root.join('spec/fixtures/files/reports/import_campaign_factors/valid_file.xlsx'),
      'application/xlsx'
    )
  end
end
