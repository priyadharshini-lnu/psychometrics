...
redis_connection = if Rails.env.development?
                     { url: 'redis://localhost:6379/0', ssl_params: ssl_params, driver: :hiredis }
                   elsif ENV.key?('REDISTOGO_URL')
                     { url: ENV['REDISTOGO_URL'], ssl_params: ssl_params, driver: :hiredis }
                   elsif ENV.key?('REDIS_URL')
                     { url: ENV['REDIS_URL'], db: 0, ssl_params: ssl_params, driver: :hiredis }
                   else
                     { path: '/var/run/redis/redis.sock', db: 0 }
                   end

Sidekiq.configure_server do |config|
  pool_size = ENV.fetch('SIDEKIQ_DB_POOL', config.concurrency + 2)
