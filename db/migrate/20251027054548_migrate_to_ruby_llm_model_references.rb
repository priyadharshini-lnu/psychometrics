...
# Generated by ruby_llm gem rake task
# rubocop:disable all
class MigrateToRubyLLMModelReferences < ActiveRecord::Migration[8.0]
  def up
    ...
    say_with_time 'Checking for additional models in existing data' do
      collect_and_create_models(chat_class, :ai_assistant_chats, model_class)
      collect_and_create_models(message_class, :ai_assistant_requests, model_class)
      model_class.count
    end

    # Migrate foreign keys
    migrate_foreign_key(:ai_assistant_chats, chat_class, model_class, :ai_model_registry)
    migrate_foreign_key(:ai_assistant_requests, message_class, model_class, :ai_model_registry)
  end

  def down
    # Remove foreign key references
    if column_exists?(:ai_assistant_requests, :ai_model_registry_id)
      remove_reference :ai_assistant_requests, :ai_model_registry, foreign_key: true
    end

    if column_exists?(:ai_assistant_chats, :ai_model_registry_id)
      remove_reference :ai_assistant_chats, :ai_model_registry, foreign_key: true
    end

    # Restore original model_id string columns
    if column_exists?(:ai_assistant_requests, :model_id_string)
      rename_column :ai_assistant_requests, :model_id_string, :model_id
    end

    if column_exists?(:ai_assistant_chats, :model_id_string)
      rename_column :ai_assistant_chats, :model_id_string, :model_id
    end
  end
