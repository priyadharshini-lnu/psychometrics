...
class AddClientIdToUser < ActiveRecord::Migration[5.1]
  def change
    ...
    add_index :users, %i[email project_id role], unique: true
    ...
    Users::Regular.includes(:clients, :memberships).select do |user|
      member = user.memberships.find { |m| %w[member manager].include?(m.role) && m.client.project? }
      if member
    ...
    Membership.includes(:user).where(role: %w[project_admin client_admin]).find_each do |m|
      user = Users::Admin.find_by(email: m.user.email, project_id: nil)
      unless user
        user = Users::Admin.new(m.user.attributes.except('role', 'id', 'authentication_token',
                                                         'project_id', 'invitation_token', 'reset_password_token'))
        user.save(validate: false)
      end
